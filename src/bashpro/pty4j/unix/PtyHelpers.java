 ...
 */
  ...
  private static final LazyValue<OSFacade> OS_FACADE_VALUE = new LazyValue<>(() -> {
    if (Platform.isMac()) {
      return new bashpro.pty4j.unix.macosx.OSFacadeImpl();
    }
    if (Platform.isFreeBSD()) {
      return new bashpro.pty4j.unix.freebsd.OSFacadeImpl();
    }
    if (Platform.isOpenBSD()) {
      return new bashpro.pty4j.unix.openbsd.OSFacadeImpl();
    }
    if (Platform.isLinux() || Platform.isAndroid()) {
      return new bashpro.pty4j.unix.linux.OSFacadeImpl();
    }
...
//    term.c_cc[VSTATUS] = CTRLKEY('T');
  ...
  public static @NotNull WinSize getWinSize(int fd, @Nullable PtyProcess process) throws UnixPtyException {
    ...
    public static int execPty(String full_path,
        ...
        return executor.execPty(full_path, argv, envp, dirpath, pts_name, fdm, err_pts_name, err_fdm, console, add_pts_name, add_pty_fdm);
    }
}